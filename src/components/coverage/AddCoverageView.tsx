...
interface AddCoverageViewProps {
  onAdd: (coverage: Omit<Coverage, 'id'>) => Promise<void>;
  onCancel: () => void;
}
...
export default function AddCoverageView({ onAdd, onCancel }: AddCoverageViewProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : 'Could not save coverage');
    } finally {
      setSubmitting(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="bg-white border border-gray-200 rounded-lg p-6 max-w-2xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div>
            ...
            <input className={inputClass} value={provider} onChange={(e) => setProvider(e.target.value)} placeholder="e.g. Aviva, Direct Line, Apple" required />
          </div>
          <div>
          ...
          <div>
            <label className={labelClass}>Excess (£)</label>
        ...
        <div className="mb-4">
          ...
          <input className={inputClass} value={limit} onChange={(e) => setLimit(e.target.value)} placeholder="e.g. £500,000 buildings / £75,000 contents" />
        </div>
        ...
        <div className="flex gap-3 justify-end">
          <button type="button" onClick={onCancel} disabled={submitting} className="px-4 py-2 bg-white border border-gray-300 rounded-md text-sm text-gray-700 cursor-pointer hover:bg-gray-50 disabled:opacity-60">
            Cancel
          </button>
          <button type="submit" disabled={submitting} className="px-6 py-2 bg-gray-900 text-white border-none rounded-md text-sm font-semibold cursor-pointer hover:bg-gray-800 disabled:opacity-60">
            {submitting ? 'Saving…' : 'Add Coverage'}
          </button>
        </div>
      </form>
    </div>
